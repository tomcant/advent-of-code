  ...
  for (const combination of getElevatorCombinations(floors[currFloorIdx])) {
    for (const nextFloorIdx of getAdjFloorIdxs({ floors, currFloorIdx })) {
      ...
      nextPosition.floors[currFloorIdx] = floors[currFloorIdx].filter(
        (component) =>
          !combination.some((c) => areComponentsEqual(c, component))
      );
      ...
      .some(
        (microchip) =>
          !generators.some((generator) =>
            areElementsEqual(generator, microchip)
          )
      );
  });
...
const cloneFloors = (floors: Floor[]): Floor[] =>
  floors.map((floor) => floor.map((component) => ({ ...component })));

const isGenerator = (component: Component): boolean =>
  ComponentType.Generator === component.type;

const isMicrochip = (component: Component): boolean =>
  ComponentType.Microchip === component.type;

const areElementsEqual = (c1: Component, c2: Component): boolean =>
  c1.element === c2.element;

const areTypesEqual = (c1: Component, c2: Component): boolean =>
  c1.type === c2.type;

const areComponentsEqual = (c1: Component, c2: Component): boolean =>
  areElementsEqual(c1, c2) && areTypesEqual(c1, c2);

const isOnBottomFloor = ({ currFloorIdx }: Position): boolean =>
  0 === currFloorIdx;

const isOnTopFloor = ({ floors, currFloorIdx }: Position): boolean =>
  floors.length - 1 === currFloorIdx;

const reachedTopFloorWithAllComponents = (pos: Position): boolean =>
  isOnTopFloor(pos) &&
  pos.floors.filter((floor) => floor.length > 0).length === 1;
