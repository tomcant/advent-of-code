...
export const part1 = (map: Map): number => {
  ...
  return findShortestPathToGoal(
    map,
    startPos,
    ({ visited }) => visited.size === numTargets
  );
};
...
const findShortestPathToGoal = (
  map: Map,
  startPos: Vec2d,
  hasGoalBeenReached: (State) => boolean
): number => {
...
const hashState = ({ pos, visited }: State): string =>
  pos.toString() + [...visited].join("");

const isTarget = (pos: Vec2d, map: Map): boolean =>
  !isPath(pos, map) && !isWall(pos, map) && +getMapTile(pos, map) > 0;

const isPath = (pos: Vec2d, map: Map): boolean =>
  Tile.Path === getMapTile(pos, map);

const isWall = (pos: Vec2d, map: Map): boolean =>
  Tile.Wall === getMapTile(pos, map);
