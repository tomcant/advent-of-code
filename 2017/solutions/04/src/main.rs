...
fn part1(passphrases: Vec<Vec<&str>>) -> i32 {
    passphrases.iter().fold(
        0, |cnt, p| cnt + (p.len() == HashSet::<&str>::from_iter(p.iter().cloned()).len()) as i32
    )
}

fn part2(passphrases: Vec<Vec<&str>>) -> i32 {
    passphrases.iter().fold(
        0, |cnt, p| cnt + (p.len() == HashSet::<String>::from_iter(p.iter().cloned().map(sort_chars)).len()) as i32
    )
}
